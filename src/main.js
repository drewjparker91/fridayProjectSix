...
async function retrieveRate(currency, usDollar) {
  ...
  if (!response) {
    ...
    return giveExchangeRate(currency, response, usDollar);
  }
}
...
function giveExchangeRate(currency, response, usDollar) {
  if (currency ===  "AUD") {
    ...
    $(".rateResult").html(`The exchange rate for the Swiss Franc is ${response.conversion_rates.CHF} and your converted total is ${(response.conversion_rates.CHF*usDollar).toFixed(2)} SFr.`);
  } else if (currency === "DKK") {
    $(".rateResult").html(`The exchange rate for the Danish Krone is ${response.conversion_rates.DKK} and your converted total is ${(response.conversion_rates.DKK*usDollar).toFixed(2)} Kr.`);
    ...
    $(".rateResult").html(`The exchange rate for the Norwegian Krone is ${response.conversion_rates.NOK} and your converted total is ${(response.conversion_rates.NOK*usDollar).toFixed(2)} kr.`);
  } else {
